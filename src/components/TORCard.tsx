...
export const TORCard: React.FC<TORCardProps> = ({ contract, onSelect }) => {
  return (
    <div 
      ...
      className="theme-card rounded-2xl p-5 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 cursor-pointer transition-all duration-200 group flex flex-col lg:flex-row gap-5 items-stretch shadow-sm"
    >
      {/* Thumbnail Image Section */}
      <div className="w-full lg:w-48 h-36 lg:h-auto shrink-0 rounded-xl overflow-hidden relative bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
        <img 
      ...
      {/* Contract Core Info Section (contract name, contract owner name, price, date) */}
      <div className="flex-1 flex flex-col justify-between">
        <div>
          ...
          <h3 className="text-base md:text-lg font-bold text-slate-900 dark:text-white group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors line-clamp-2 leading-snug mb-2">
            {contract.title}
          </h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-200 dark:border-slate-800">
          <div>
            ...
            <span className="text-lg font-extrabold text-sky-700 dark:text-sky-300">{contract.priceFormatted}</span>
          </div>
          
          <div className="flex items-center gap-1 text-xs font-semibold text-sky-600 dark:text-sky-400 group-hover:translate-x-1 transition-transform">
      ...
      {/* Right Section: คุณสมบัติ (Qualifications / Properties Checklist) */}
      <div className="w-full lg:w-72 shrink-0 bg-slate-50 dark:bg-slate-950/80 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
        <div>
            ...
            {contract.properties.slice(0, 3).map((prop, idx) => (
              <div key={prop.id || idx} className="flex items-start gap-2 text-xs">
                {prop.fulfilledBySoftwareHouse ? (
                  ...
                  <XCircle className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 shrink-0 mt-0.5" />
                )}
                <span className={`line-clamp-2 ${prop.fulfilledBySoftwareHouse ? 'text-slate-800 dark:text-slate-200 font-medium' : 'text-slate-500 dark:text-slate-400'}`}>
                  {prop.property}
                </span>
              </div>
            ))}
            {contract.properties.length > 3 && (
              <p className="text-[11px] text-sky-600 dark:text-sky-400 font-medium italic pl-5">
